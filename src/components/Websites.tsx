          ...
          <span
            ...
            style={
              light
                ...
                : { background: 'var(--tag-bg-2)', color: 'var(--tag-color-2)', border: '1px solid var(--tag-border-2)' }
            }
          >
            {tag}
          </span>
          ...
          <span
            ...
            style={
              light
                ...
                : { background: 'var(--tag-bg-1)', color: 'var(--tag-color-1)', border: '1px solid var(--tag-border-1)' }
            }
          >
            +{remainCount}
          </span>
    ...
    <section className="min-h-[calc(100vh-7rem)] pt-24 pb-4 px-4 sm:px-6 lg:px-8 max-w-6xl mx-auto">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-12">
        ...
        <h2
          ...
          style={{ color: 'var(--primary)', textShadow: '0 0 24px var(--primary-glow)' }}
        >
          作品集
        </h2>
        <div className="section-divider w-20 mx-auto mb-4" />
        <p className="max-w-xl mx-auto text-sm font-medium" style={{ color: 'var(--text-dim)' }}>
          我用代码和热情构建的数字作品与应用
        </p>
      </motion.div>
          ...
          >
            {project.image ? (
                ...
                <div className="absolute bottom-0 left-0 right-0 p-5">
                  ...
                  {project.url && (
                    <a
                      ...
                      style={{ color: 'var(--tag-color-1)' }}
                      onMouseEnter={(e) => (e.currentTarget.style.color = 'var(--tag-color-3)')}
                      onMouseLeave={(e) => (e.currentTarget.style.color = 'var(--tag-color-1)')}
                    >
              ...
              <div className="glass-panel p-5 rounded-2xl h-full flex flex-col">
                ...
                <h3 className="text-lg font-black mb-2" style={{ color: 'var(--text-bright)' }}>{project.title}</h3>
      ...
      {PROJECTS.length === 0 && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-20">
          <div
            ...
            style={{ background: 'rgba(167,139,250,0.1)', border: '1px solid rgba(167,139,250,0.2)', color: 'var(--text-dim)' }}
          >
            <Box size={40} />
          </div>
          <p className="font-medium" style={{ color: 'var(--text-dim)' }}>暂无作品</p>
        </motion.div>
      )}
    </section>
  );
